...
document.querySelector('#formulario-reservar').addEventListener('submit', (e)=>{
    e.preventDefault()
        ...
        if(sku==libro.sku){
            ...
            if(libro.reservado==0){
                ...
                boton.addEventListener('click', ()=>{
                    ...
                    libro.reservado=1
                    sacaTooltip('Libro reservado con éxito.')
                    //Borramos el contenido del padre una vez reservado el libro
                    seleccionado.innerHTML=''
                    //Y guardamos en localStorage
                    localStorage.setItem('biblioteca_libros', JSON.stringify(libros))
                    //Reseteamos el formulario
                    document.querySelector('#formulario-reservar').reset()
                })
            }else{
                sacaTooltip('Este libro ya está reservado.')
            }
